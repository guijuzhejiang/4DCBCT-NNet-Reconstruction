...
from monai.transforms import (
    Compose, LoadImaged, EnsureChannelFirstd, ScaleIntensityRanged, RandHistogramShiftd, RandBiasFieldd,
...
class NnetTrainer:
    ...
    def setup_optimizer(self):
        ...
        weight_grad = TRAINING_CONFIG['weight_grad']
        ...
        # 边缘/梯度损失：用 Kornia 的 spatial_gradient (sobel)
        def gradient_loss(pred, target):
            # pred, target: tensors (N, C, H, W)  (C can be 1 or >1)
            # compute spatial gradients shape (N,C,2,H,W) -> take magnitude or axis-wise difference
            g_pred = kornia.filters.spatial_gradient(pred, mode='sobel')  # (N,C,2,H,W)
            g_tgt = kornia.filters.spatial_gradient(target, mode='sobel')
            # compute L1 between gradient components, average over channels
            loss = torch.mean(torch.abs(g_pred - g_tgt))
            return loss

        # 損失関数を結合
        def combined_loss(pred, target, return_components=False):
            mse, l1, ssim, percep, grad = 0, 0, 0, 0, 0
            if weight_mse > 0:
                mse = weight_mse * self.mse_loss(pred, target)
            if weight_l1 > 0:
                l1 = weight_l1 * self.L1_loss(pred, target)
            if weight_ssim > 0:
                ssim = weight_ssim * self.ssim_loss(pred, target)
            if weight_percep > 0:
                percep = weight_percep * self.perceptual_loss(pred, target)
            if weight_grad > 0:
                grad = weight_grad * gradient_loss(pred, target)
            loss_total = mse + l1 + percep + ssim + grad
            ...
            if return_components:
                return loss_total, {
                    ...
                    'percep': percep.item() if isinstance(percep, torch.Tensor) else percep,
                    'grad': grad.item() if isinstance(grad, torch.Tensor) else grad
                }
            return loss_total
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            try:
                ...
                loss, loss_components = self.criterion(prediction, labels, return_components=True)
                # 逆伝播
                loss.backward()
                # 勾配クリッピング
                ...
                if i % 10 == 0:
                    ...
                    print(
                        ...
                        f"SSIM: {loss_components['ssim']:.6f}, "
                        f"Percep: {loss_components['percep']:.6f}, "
                        f"Grad: {loss_components['grad']:.6f}, "
                        f"RMSE: {batch_metrics['rmse']:.6f}, "
                        f"MAE: {batch_metrics['mae']:.6f}, "
                        f"PSNR: {batch_metrics['psnr']:.6f}, "
                        f"SSIM_metric: {batch_metrics['ssim']:.6f}, "
                    ...
                    if self.tb_writer:
                        ...
                        self.tb_writer.add_scalar('Train/MSE_Loss', loss_components['mse'], global_step)
                        self.tb_writer.add_scalar('Train/L1_Loss', loss_components['l1'], global_step)
                        self.tb_writer.add_scalar('Train/SSIM_Loss', loss_components['ssim'], global_step)
                        self.tb_writer.add_scalar('Train/Perceptual_Loss', loss_components['percep'], global_step)
                        self.tb_writer.add_scalar('Train/Gradient_Loss', loss_components['grad'], global_step)
                    ...
                    if self.use_wandb:
                        wandb.log({
                            ...
                            'train_mse_loss': loss_components['mse'],
                            'train_l1_loss': loss_components['l1'],
                            'train_ssim_loss': loss_components['ssim'],
                            'train_perceptual_loss': loss_components['percep'],
                            'train_gradient_loss': loss_components['grad'],
    ...
    def validate_epoch(self, epoch):
        ...
        self.model.eval()
        running_loss = 0.0
        running_loss_components = {'mse': 0.0, 'l1': 0.0, 'ssim': 0.0, 'percep': 0.0, 'grad': 0.0}
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                for key in running_loss_components:
                    running_loss_components[key] += loss_components[key]
                ...
                # アーチファクトの可視化
                if self.tb_writer:
                    # filename_or_objリストの中で'axl0047.img'を含むファイルを検索
                    filenames = batch["img"].meta['filename_or_obj']
                    for idx, filename in enumerate(filenames):
                        sub_names = filename.split('/')
                        if 'axl0047.img' == sub_names[-1]:
                            subject = sub_names[-4]
                            
                            # アーチファクト除去効果を可視化
                            # データをCPUに移動し、浮動小数点型に変換
                            img_cpu = images[idx].cpu().float()  # (1,512,512)
                            prior_cpu = prior[idx].cpu().float()  # (1,512,512)
                            label_cpu = labels[idx].cpu().float()  # (1,512,512)
                            pred_cpu = prediction[idx].cpu().float()  # (1,512,512)
                            ...
                            # subject名を含むtagでTensorBoardに記録（互いに覆われることを避ける）
                            self.tb_writer.add_image(f"Validation/{subject}_axl0047_Input_Prior_GT_Pred", grid, epoch)

        # 平均を計算
        self.last_val_loss = running_loss / num_batches
        avg_loss_components = {k: v / num_batches for k, v in running_loss_components.items()}
        ...
        print(
            ...
            f"Val_MSE: {avg_loss_components['mse']:.6f}, "
            f"Val_L1: {avg_loss_components['l1']:.6f}, "
            f"Val_SSIM_Loss: {avg_loss_components['ssim']:.6f}, "
            f"Val_Percep: {avg_loss_components['percep']:.6f}, "
            f"Val_Grad: {avg_loss_components['grad']:.6f}, "
        ...
        if self.tb_writer:
            ...
            self.tb_writer.add_scalar('Val/MSE_Loss', avg_loss_components['mse'], epoch)
            self.tb_writer.add_scalar('Val/L1_Loss', avg_loss_components['l1'], epoch)
            self.tb_writer.add_scalar('Val/SSIM_Loss', avg_loss_components['ssim'], epoch)
            self.tb_writer.add_scalar('Val/Perceptual_Loss', avg_loss_components['percep'], epoch)
            self.tb_writer.add_scalar('Val/Gradient_Loss', avg_loss_components['grad'], epoch)
        ...
        if self.use_wandb:
            wandb.log({
                ...
                'val_mse_loss': avg_loss_components['mse'],
                'val_l1_loss': avg_loss_components['l1'],
                'val_ssim_loss': avg_loss_components['ssim'],
                'val_perceptual_loss': avg_loss_components['percep'],
                'val_gradient_loss': avg_loss_components['grad'],
